...
async def summarize_papers(papers: list[Paper], config: Config, priority_blogs: list[Paper] = None) -> str:
    ...
    extraction_settings = ExtractionSettings(
        ...
        selection_mode=getattr(config, "paper_evidence_selection_mode", "section_balanced"),
        main_body_fallback_pages=getattr(config, "main_body_fallback_pages", 10),
    ...
    summarizer = PaperSummarizer(
        ...
        synthesis_max_output_tokens=getattr(config, "synthesis_max_output_tokens", 16384),
        synthesis_reasoning_effort=getattr(config, "synthesis_reasoning_effort", ""),
